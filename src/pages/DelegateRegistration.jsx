...
>>>>>>> 5e8146ace9c931cf3e46732cbd76b979fb48f4b9
    );
    navigate("/", { replace: true });
  }, [canAccessDelegate, entitlementsLoading, navigate, popup]);
    ...
    const hydrateProfile = async () => {
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          name: String(
            data?.name || prev.name || authUser.displayName || ""
          ).trim(),
          email: String(
            data?.email || authUser.email || prev.email || ""
          ).trim(),
          phone: String(data?.phone || prev.phone || "").trim(),
          college: String(data?.college || prev.college || "").trim(),
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (missingProfile) {
        popup.info(
          "Please update your Profile (name, phone, college) before registering."
        );
        navigate("/profile");
        return;
      }
  ...
  const handlePayment = async () => {
    ...
    if (!willUseNewTab) {
      popup.info(
        "Popup blocked in your browser. Continuing to payment in this tab…"
      );
    }

    setLoading(true);
    const endpoint = `${BASE_API_URL}/delegate/register/self`;
    ...
    try {
      ...
      closePaymentTab();
      popup.error(
        `Payment failed: ${data.message || "No payment URL returned"}`
      );
    } catch (error) {
      console.error("Payment Error:", error);
      closePaymentTab();
            ...
            <>
              ...
              {submitted ? (
                    ...
                    <>
                      ...
                      {paymentStatus === PaymentStatus.Pending && (
                        <div className="mb-6 border-l-4 border-yellow-400 bg-yellow-900/40 p-4">
                          <div className="flex">
                            ...
                            <div className="ml-3">
                              <p className="text-sm text-yellow-100">
                                Payment is pending. Please complete your
                                payment.
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex items-start gap-4 font-mono">
                          ...
                          <div className="relative flex-1 break-words py-1 text-base text-white">
                            <div className="min-h-[1.5em] border-b border-white pb-1">
                              {formData.college}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="mt-10 border-t border-white pt-6">
                        ...
                        <div className="mb-6 flex items-center gap-3">
                          ...
                          <label
                            htmlFor="terms"
                            className="cursor-pointer select-none text-xs text-gray-300 md:text-sm"
                          >
                            I agree to the{" "}
                        ...
                        <button
                          className={`w-full font-mono text-lg font-bold uppercase tracking-widest transition-colors py-4 ${
                            loading
                ...
                <form
                  ...
                  className="relative space-y-6 rounded-[28px] border border-white/12 bg-black/55 p-6 md:p-10 shadow-[0_40px_120px_rgba(255,0,48,0.25)] backdrop-blur-xl"
                >
                  {(checkingGroupStatus || inGroupDelegate) && (
                    <div className="rounded-2xl border border-white/12 bg-white/5 p-4 text-sm text-white/80 backdrop-blur-lg">
                      {checkingGroupStatus ? (
                  ...
                  <div className="pt-6">
                    <button
                      type="submit"
                      disabled={
                        loading || checkingGroupStatus || inGroupDelegate
                      }
